	...
	def _get_report_values(self, docids, data=None):
		...
		for product in products:
			...
			if product.qty_enable > 0:
				exist = True
		if exist == True:
			interests = self.env['account.financing'].search([], order="id desc", limit=1)
			return {
				'doc_ids': data['ids'],
				'doc_model': data['model'],
				'docs': products_list,
				'mont_3': interests[0].mont_3,
				'mont_6': interests[0].mont_6,
				'mont_12': interests[0].mont_12,
				'mont_18': interests[0].mont_18
			}
		else:
			raise exceptions.Warning("Sin inventario")

class AccountBankFinancing(models.Model):
	...
	mont_18 = fields.Float(string='18 meses')
